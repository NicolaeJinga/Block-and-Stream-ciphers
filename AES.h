...
public:
	unsigned char Message[16] = { 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff };
	...
	unsigned int KeySchedule[60]; //60 = (Nb=4)*((Nr=14)+1) for 256 bit key.
	...
	void SetMessage(unsigned char in[]);
	void CopyStateToOut(unsigned char out[]);

	unsigned char* Encipher();
	unsigned char* Decipher();

	unsigned char InitVector[16];
	unsigned char CBCCarryOver[16];
	unsigned char DecryptCBCCarryOver[16];
	...
	int LoadKey(char* fileInput, bool loadAsHex = false);
	void SetKey(char* key, bool hex);
